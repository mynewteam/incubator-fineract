...
public class StaffReadPlatformServiceImpl implements StaffReadPlatformService {
    ...
    private static final class StaffMapper implements RowMapper<StaffData> {
        ...
        public StaffData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return StaffData.instance(id, firstname, lastname, displayName, officeId, officeName, isLoanOfficer,
                    externalId, mobileNo, isActive, joiningDate);
        }
    }
    ...
    private static final class StaffInOfficeHierarchyMapper implements RowMapper<StaffData> {
        ...
        public String schema(final boolean loanOfficersOnly) {
            ...
            sqlBuilder.append(
                    "s.display_name as displayName, s.is_loan_officer as isLoanOfficer, s.external_id as externalId, ");
            ...
            if (loanOfficersOnly) {
                sqlBuilder.append("and s.is_loan_officer = 1 ");
            }
    ...
    public Collection<StaffData> retrieveAllStaffForDropdown(final Long officeId) {
        ...
        // adding the Authorization criteria so that a user cannot see an employee who
        // does not belong to his office or a sub office for his office.
        final String hierarchy = this.context.authenticatedUser().getOffice().getHierarchy() + "%";

        final Long defaultOfficeId = defaultToUsersOfficeIfNull(officeId);

        final String sql = "select " + this.lookupMapper.schema()
    ...
    public Collection<StaffData> retrieveAllStaff(final String sqlSearch, final Long officeId,
            final boolean loanOfficersOnly, final String status) {
    ...
    private String getStaffCriteria(final String sqlSearch, final Long officeId, final boolean loanOfficersOnly,
        ...
        if (loanOfficersOnly) {
            extraCriteria.append(" and s.is_loan_officer = 1 ");
        }
    ...
    @Override
    public Collection<StaffData> retrieveAllStaffInOfficeAndItsParentOfficeHierarchy(final Long officeId,
    ...
    public Object[] hasAssociatedItems(final Long staffId) {
        ...
        String sql = "select c.display_name as client, g.display_name as grp,l.loan_officer_id as loan, s.field_officer_id as sav"
                ...
                + " where  staff.id  = ? " + " group by staff.id";

        List<Map<String, Object>> result = this.jdbcTemplate.queryForList(sql,
                new Object[] { ClientStatus.CLOSED.getValue(), LoanStatus.WITHDRAWN_BY_CLIENT.getValue(),
                        SavingsAccountStatusType.WITHDRAWN_BY_APPLICANT.getValue(), staffId });
        if (result != null) {
