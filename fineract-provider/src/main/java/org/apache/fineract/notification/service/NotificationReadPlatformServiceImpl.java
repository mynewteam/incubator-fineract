...
public class NotificationReadPlatformServiceImpl implements NotificationReadPlatformService {
    ...
    private boolean checkForUnreadNotifications(Long appUserId) {
        String sql = "SELECT id, notification_id as notificationId, user_id as userId, is_read as isRead, created_at "
                + "as createdAt FROM notification_mapper WHERE user_id = ? AND is_read = 0";
        List<NotificationMapperData> notificationMappers = this.jdbcTemplate.query(sql, notificationMapperRow,
                appUserId);
    ...
    private Page<NotificationData> getNotificationDataPage(SearchParameters searchParameters, Long appUserId,
            String sql) {
        ...
        return this.paginationHelper.fetchPage(this.jdbcTemplate, sqlCountRows, sqlBuilder.toString(), params,
                this.notificationDataRow);
    }
