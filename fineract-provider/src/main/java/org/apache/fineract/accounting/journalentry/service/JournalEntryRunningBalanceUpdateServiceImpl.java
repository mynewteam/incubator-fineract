...
public class JournalEntryRunningBalanceUpdateServiceImpl implements JournalEntryRunningBalanceUpdateService {
    ...
    // if a limit is not added to the running balance select statements below and
    // the resultset is more than 400,000,
    // the script will eat up all of the server memory
    private final String selectRunningBalanceSqlLimit = " OFFSET 0 ROWS FETCH NEXT 10000 ROWS ONLY";
    ...
    private final String officeRunningBalanceSql = "select je.office_running_balance as runningBalance,je.account_id as accountId from acc_gl_journal_entry je "
            ...
            + "inner join (select max(entry_date) as datetime from acc_gl_journal_entry where office_id=? and entry_date < ? group by account_id) je3 "
    ...
    private final String organizationRunningBalanceSql = "select je.organization_running_balance as runningBalance,je.account_id as accountId from acc_gl_journal_entry je "
            ...
            + "inner join (select max(entry_date) as datetime from acc_gl_journal_entry where entry_date < ? group by account_id) je3 "
            + "where je2.id = je.id and je.entry_date = je3.date group by je.id order by je.entry_date DESC "
            + selectRunningBalanceSqlLimit;
    ...
    private final String officesRunningBalanceSql = "select je.office_running_balance as runningBalance,je.account_id as accountId,je.office_id as officeId "
            ...
            + "inner join (select max(entry_date) as datetime from acc_gl_journal_entry where entry_date < ? group by office_id,account_id) je3 "
            + "where je2.id = je.id and je.entry_date = je3.date group by je.id order by je.entry_date DESC "
            + selectRunningBalanceSqlLimit;
    ...
    private void updateOrganizationRunningBalance(Date entityDate) {
        ...
        List<Map<String, Object>> list = jdbcTemplate.queryForList(organizationRunningBalanceSql, entityDate,
                entityDate);
        for (Map<String, Object> entries : list) {
            Long accountId = Long.parseLong(entries.get("accountId").toString()); // Drizzle is returning Big Integer
                                                                                  // where as MySQL returns Long.
            if (!runningBalanceMap.containsKey(accountId)) {
                runningBalanceMap.put(accountId, (BigDecimal) entries.get("runningBalance"));
            }
        }

        List<Map<String, Object>> officesRunningBalanceList = jdbcTemplate.queryForList(officesRunningBalanceSql,
        ...
        List<JournalEntryData> entryDatas = jdbcTemplate.query(entryMapper.organizationRunningBalanceSchema(),
                entryMapper, new Object[] { entityDate });
        if (entryDatas.size() > 0) {
            ...
            for (int index = 0; index < entryDatas.size(); index++) {
                ...
                String sql = "UPDATE acc_gl_journal_entry je SET je.is_running_balance_calculated=1, je.organization_running_balance="
                        + runningBalance + ",je.office_running_balance=" + officeRunningBalance + " WHERE  je.id="
                        + entryData.getId();
                updateSql.add(sql);
                batchIndex++;
    ...
    private void updateRunningBalance(Long officeId, Date entityDate) {
        ...
        List<Map<String, Object>> list = jdbcTemplate.queryForList(officeRunningBalanceSql, officeId, entityDate,
                officeId, entityDate);
        for (Map<String, Object> entries : list) {
        ...
        for (JournalEntryData entryData : entryDatas) {
            BigDecimal runningBalance = calculateRunningBalance(entryData, runningBalanceMap);
            String sql = "UPDATE acc_gl_journal_entry je SET je.office_running_balance=" + runningBalance
                    + " WHERE  je.id=" + entryData.getId();
            updateSql[i++] = sql;
        }
    ...
    private static final class GLJournalEntryMapper implements RowMapper<JournalEntryData> {
        ...
        public JournalEntryData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
            ...
            return new JournalEntryData(id, officeId, null, null, glAccountId, null, accountType, null, entryType,
                    amount, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
        }
    }
