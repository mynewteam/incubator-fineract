...
public class SpotRateCommand
{
    ...
    public SpotRateCommand(final String currency_code, final double buyingRate, final double sellingRate, 
    		final double spotRate, final LocalDate transactionDate) {
	...
	public void validateForCreate() {
        ...
        baseDataValidator.reset().parameter(SpotRateJsonInputParams.CURRENCY_CODE.getValue()).value(this.currency_code).notBlank().notExceedingLengthOf(5);

        baseDataValidator.reset().parameter(SpotRateJsonInputParams.BUYING_RATE.getValue()).value(this.buyingRate).notNull()
        	.integerGreaterThanZero();

        baseDataValidator.reset().parameter(SpotRateJsonInputParams.SELLING_RATE.getValue()).value(this.sellingRate).notNull()
                .integerGreaterThanZero();

        baseDataValidator.reset().parameter(SpotRateJsonInputParams.SPOTRATE.getValue()).value(this.spotRate).notNull()
        		.integerGreaterThanZero();

        baseDataValidator.reset().parameter(SpotRateJsonInputParams.TRANSACTION_DATE.getValue()).value(this.transactionDate).notNull()
