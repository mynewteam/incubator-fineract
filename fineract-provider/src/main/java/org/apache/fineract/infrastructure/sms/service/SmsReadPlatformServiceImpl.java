...
public class SmsReadPlatformServiceImpl implements SmsReadPlatformService {
    ...
    private static final class SmsMapper implements RowMapper<SmsData> {
        ...
        public SmsData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return SmsData.instance(id, groupId, clientId, staffId, status, mobileNo, message, providerId,
                    campaignName);
        }
    }
    ...
    @Override
    public Page<SmsData> retrieveSmsByStatus(final Long campaignId, final SearchParameters searchParameters,
            final Integer status, final Date dateFrom, final Date dateTo) {
