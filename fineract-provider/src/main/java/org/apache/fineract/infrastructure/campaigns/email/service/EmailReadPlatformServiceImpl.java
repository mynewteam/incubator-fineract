...
public class EmailReadPlatformServiceImpl implements EmailReadPlatformService {
    ...
    private static final class EmailMapper implements RowMapper<EmailData> {
        ...
        public EmailData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return EmailData.instance(id, groupId, clientId, staffId, status, emailAddress, emailSubject, message, null,
                    null, null, null, null, campaignName, sentDate, errorMessage);
        }
    }
    ...
    public Collection<EmailData> retrieveAllPending(final SearchParameters searchParameters) {
        final String sqlPlusLimit = (searchParameters.getLimit() > 0) ? " fetch next " + searchParameters.getLimit()
                : "";
    ...
    @Override
    public Page<EmailData> retrieveEmailByStatus(final Integer limit, final Integer status, final Date dateFrom,
            final Date dateTo) {
        ...
        final String sqlPlusLimit = (limit > 0) ? " offset rows 0 " + limit : "";
