...
public class JDBCDriverConfig {
	 private final static Logger logger = LoggerFactory.getLogger(JDBCDriverConfig.class);
	...
	public String constructProtocol(String schemaServer, String schemaServerPort, String schemaName) {
		...
		// StringBuilder(protocol).append(":").append(subProtocol).append("://").append(schemaServer).append(':').append(schemaServerPort)
		// .append('/').append(schemaName).toString();
		final String url = new StringBuilder(protocol).append(":").append(subProtocol).append(":thin:@")
				.append(schemaServer).append(":").append(schemaServerPort).append(":").append(schemaName).toString();
		logger.debug("ORCLJdbc : "+url);
		return url;
	}
