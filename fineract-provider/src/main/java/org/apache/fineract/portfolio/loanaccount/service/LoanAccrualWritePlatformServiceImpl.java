            ...
            if (accrualData.getDueDateAsLocaldate().isAfter(tilldate)) {  // Running back date accrual
                ...
                if (accruredTill == null || accruredTill.isBefore(tilldate)) {
                	...
                	if(accruredTill!=null) {
                		logger.debug("--- if (accruredTill == "+accruredTill+" || accruredTill.isBefore("+tilldate+":tilldate)) { Number 2:");
                	}
                	else {
                		logger.debug("--- if (accruredTill == null || accruredTill.isBefore("+tilldate+":tilldate)) { Number 2:");
                	}
...
//              addAccrualAccounting(accrualData);
    ...
    private void addAccrualAccounting(LoanScheduleAccrualData scheduleAccrualData, BigDecimal amount,
    	...
    	logger.trace("-- accruedTill :" + accruedTill + ":Accrual on LoanID:("+scheduleAccrualData.getLoanId()+")--");
...
//                        + "            BigDecimal totalAccInterest, BigDecimal feeportion, BigDecimal totalAccFee:"
//                        + totalAccFee + ", BigDecimal penaltyportion:" + penaltyportion + ",\n"
//                        + "            BigDecimal totalAccPenalty:" + totalAccPenalty + ", final LocalDate accruedTill:"
//                        + accruedTill.toString() + ")");
        ...
        for (Map.Entry<LoanChargeData, BigDecimal> entry : applicableCharges.entrySet()) {
            ...
            this.jdbcTemplate.update(chargespaidSql, transactonId, chargeData.getId(), entry.getValue(),scheduleAccrualData.getInstallmentNumber());
        }
        ...
        this.jdbcTemplate.update(repaymetUpdatesql, totalAccInterest, totalAccFee, totalAccPenalty, scheduleAccrualData.getRepaymentScheduleId());
