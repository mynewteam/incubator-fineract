...
public class ClientFamilyMemberCommandFromApiJsonDeserializer
{
	...
	private final Set<String> supportedParameters = new HashSet<>(
		Arrays.asList("id", "clientId", "firstName", "middleName", "lastName", "qualification", "mobileNumber",
			"age", "isDependent", "relationshipId", "maritalStatusId", "genderId", "dateOfBirth", "professionId",
			"locale", "dateFormat", "familyMembers"));
	...
	public void validateForCreate(String json)
	{
		...
		{
			baseDataValidator.reset().value(this.fromApiJsonHelper.extractJsonArrayNamed("familyMembers", element))
				.arrayNotEmpty();
		}
	...
	public void validateForCreate(final long clientId, String json)
	{
		...
		if (this.fromApiJsonHelper.extractStringNamed("firstName", element) != null)
		{
			...
			baseDataValidator.reset().parameter("firstName").value(firstName).notNull().notBlank()
				.notExceedingLengthOf(100);
		} else
		{
			baseDataValidator.reset().parameter("firstName")
				.value(this.fromApiJsonHelper.extractStringNamed("firstName", element)).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractStringNamed("lastName", element) != null)
		{
			...
			baseDataValidator.reset().parameter("lastName").value(lastName).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractStringNamed("middleName", element) != null)
		{
			...
			baseDataValidator.reset().parameter("middleName").value(middleName).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractStringNamed("qualification", element) != null)
		{
			...
			baseDataValidator.reset().parameter("qualification").value(qualification).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractStringNamed("mobileNumber", element) != null)
		{
			...
			baseDataValidator.reset().parameter("mobileNumber").value(mobileNumber).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractBooleanNamed("isDependent", element) != null)
		{
			...
			baseDataValidator.reset().parameter("isDependent").value(isDependent).notNull().notBlank()
				.notExceedingLengthOf(100);
		}
		...
		if (this.fromApiJsonHelper.extractLongNamed("relationShipId", element) != null)
		{
			...
			baseDataValidator.reset().parameter("relationShipId").value(relationShipId).notBlank()
				.longGreaterThanZero();

		} else
		{
			baseDataValidator.reset().parameter("relationShipId")
				.value(this.fromApiJsonHelper.extractLongNamed("relationShipId", element)).notBlank()
				.longGreaterThanZero();
		}
		...
		if (this.fromApiJsonHelper.extractLongNamed("maritalStatusId", element) != null)
		{
			...
			baseDataValidator.reset().parameter("maritalStatusId").value(maritalStatusId).notBlank()
				.longGreaterThanZero();
	...
	public void validateForUpdate(final long familyMemberId, String json)
	{
		...
		if (this.fromApiJsonHelper.extractLocalDateNamed("dateOfBirth", element) != null)
		{
			...
			final LocalDate dateOfBirth = this.fromApiJsonHelper.extractLocalDateNamed("dateOfBirth", element);
			baseDataValidator.reset().parameter("dateOfBirth").value(dateOfBirth)
