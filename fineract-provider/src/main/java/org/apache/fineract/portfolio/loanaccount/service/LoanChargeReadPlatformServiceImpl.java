...
public class LoanChargeReadPlatformServiceImpl implements LoanChargeReadPlatformService {
	...
	private static final class LoanChargeMapper implements RowMapper<LoanChargeData> {
		...
		public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return new LoanChargeData(id, chargeId, name, currency, amount, amountPaid, amountWaived, amountWrittenOff,
					amountOutstanding, chargeTimeType, dueAsOfDate, chargeCalculationType, percentageOf,
					amountPercentageAppliedTo, penalty, paymentMode, paid, waived, null, minCap, maxCap,
	...
	@Override
	public Collection<LoanChargeData> retrieveLoanChargesForFeePayment(final Integer paymentMode,
	...
	private static final class LoanChargeMapperWithLoanId implements RowMapper<LoanChargeData> {
		...
		public String schema() {
			return "lc.id as id, lc.due_for_collection_as_of_date as dueAsOfDate, "
					+ "lc.amount_outstanding_derived as amountOutstanding, " + "lc.charge_time_enum as chargeTime, "
					+ "loan.id as loanId " + "from  m_loan_charge lc " + "join m_loan loan on loan.id = lc.loan_id ";
		}
	...
	@Override
	public Collection<LoanInstallmentChargeData> retrieveInstallmentLoanCharges(Long loanChargeId,
			boolean onlyPaymentPendingCharges) {
	...
	private static final class LoanInstallmentChargeMapper implements RowMapper<LoanInstallmentChargeData> {
		...
		public String schema() {
			return " lsi.installment as installmentNumber, lsi.duedate as dueAsOfDate, "
					+ "lic.amount_outstanding_derived as amountOutstanding," + "lic.amount as  amount, "
					+ "lic.is_paid_derived as paid, " + "lic.amount_waived_derived as amountWaived, "
					+ "lic.waived as waied " + "from  m_loan_installment_charge lic "
					+ "join m_loan_repayment_schedule lsi on lsi.id = lic.loan_schedule_id ";
		}

		@Override
		public LoanInstallmentChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
				throws SQLException {
			...
			return new LoanInstallmentChargeData(installmentNumber, dueAsOfDate, amount, amountOutstanding,
					amountWaived, paid, waived);
		}
	}
	...
	private static final class LoanChargeAccrualMapper implements RowMapper<LoanChargeData> {
		...
		public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return new LoanChargeData(id, chargeId, dueAsOfDate, chargeTimeType, amount, amountAccrued, amountWaived,
					penalty);
		}
	}
	...
	private Collection<LoanChargeData> updateLoanChargesWithUnrecognizedIncome(final Long loanId,
		...
		return this.jdbcTemplate.query(sql, rm,
				new Object[] { LoanTransactionType.WAIVE_CHARGES.getValue(), loanId, loanId });
	...
	private static final class LoanInstallmentChargeAccrualMapper implements RowMapper<LoanInstallmentChargeData> {
		...
		public LoanInstallmentChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return new LoanInstallmentChargeData(installmentNumber, dueAsOfDate, amount, amountOutstanding,
					amountWaived, paid, waived, amountAccrued);
		}
	}

	private Collection<LoanInstallmentChargeData> updateInstallmentLoanChargesWithUnrecognizedIncome(
			final Long loanChargeId, final Map<Integer, LoanInstallmentChargeData> installmentChargeDatas) {
		final LoanInstallmentChargeUnRecognizedIncomeMapper rm = new LoanInstallmentChargeUnRecognizedIncomeMapper(
				installmentChargeDatas);
		String sql = "select " + rm.schema() + " where cpb.loan_charge_id = ? group by cpb.installment_number  ";
		return this.jdbcTemplate.query(sql, rm,
				new Object[] { LoanTransactionType.WAIVE_CHARGES.getValue(), loanChargeId });
	}
	...
	@Override
	public Collection<LoanChargePaidByData> retriveLoanChargesPaidBy(Long chargeId,
			final LoanTransactionType transactionType, final Integer installmentNumber) {
	...
	private static final class LoanChargesPaidByMapper implements RowMapper<LoanChargePaidByData> {
		...
		@Override
		public LoanChargePaidByData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
				throws SQLException {
