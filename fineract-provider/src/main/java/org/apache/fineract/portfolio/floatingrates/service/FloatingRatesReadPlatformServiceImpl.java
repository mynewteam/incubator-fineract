...
public class FloatingRatesReadPlatformServiceImpl implements FloatingRatesReadPlatformService {
	...
	private final class FloatingRateRowMapper implements RowMapper<FloatingRateData> {
		...
		private final StringBuilder sqlQuery = new StringBuilder().append("rate.id as id, ")
				.append("rate.name as name, ").append("rate.is_base_lending_rate as isBaseLendingRate, ")
				.append("rate.is_active as isActive, ").append("crappu.username as createdBy, ")
				.append("rate.created_date as createdOn, ").append("moappu.username as modifiedBy, ")
				.append("rate.lastmodified_date as modifiedOn ").append("FROM m_floating_rates rate ")
				.append("LEFT JOIN m_appuser  crappu on rate.createdby_id = crappu.id ")
				.append("LEFT JOIN m_appuser  moappu on rate.lastmodifiedby_id = moappu.id ");
		...
		public FloatingRateData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return new FloatingRateData(id, name, isBaseLendingRate, isActive, createdBy, createdOn, modifiedBy,
	...
	private final class FloatingRatePeriodRowMapper implements RowMapper<FloatingRatePeriodData> {
		...
		private final StringBuilder sqlQuery = new StringBuilder().append("period.id as id, ")
				.append("period.from_date as fromDate, ").append("period.interest_rate as interestRate, ")
				...
				.append("period.lastmodified_date as modifiedOn ").append("FROM m_floating_rates_periods period ")
				.append("LEFT JOIN m_appuser crappu on period.createdby_id = crappu.id ")
				.append("LEFT JOIN m_appuser moappu on period.lastmodifiedby_id = moappu.id ");
		...
		public FloatingRatePeriodData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			final String modifiedBy = rs.getString("modifiedBy");
			final LocalDate modifiedOn = JdbcSupport.getLocalDate(rs, "modifiedOn");
			return new FloatingRatePeriodData(id, fromDate, interestRate, isDifferentialToBaseLendingRate, isActive,
	...
	private final class FloatingRateLookupMapper implements RowMapper<FloatingRateData> {
		...
		private final StringBuilder sqlQuery = new StringBuilder().append("rate.id as id, ")
				.append("rate.name as name, ").append("rate.is_base_lending_rate as isBaseLendingRate ")
				.append("FROM m_floating_rates rate ");
	...
	private final class FloatingInterestRatePeriodRowMapper implements RowMapper<InterestRatePeriodData> {
		...
		private final StringBuilder sqlQuery = new StringBuilder().append("select ")
				...
				.append("    baserate.interest_rate as baserate_interest_rate ").append(" from m_product_loan  lp ")
				.append(" join m_product_loan_floating_rates plfr on lp.id = plfr.loan_product_id ")
				.append(" join  m_floating_rates linkedrate on linkedrate.id = plfr.floating_rates_id ")
				.append("left join m_floating_rates_periods linkedrateperiods on (linkedrate.id = linkedrateperiods.floating_rates_id and linkedrateperiods.is_active = 1) ")
				.append("left join ( ").append("    select blr.name, ").append("    blr.is_base_lending_rate, ")
				.append("    blr.is_active, ").append("    blrperiods.from_date, ")
				.append("    blrperiods.interest_rate ").append("    from m_floating_rates blr ")
				.append("    left join m_floating_rates_periods blrperiods on (blr.id = blrperiods.floating_rates_id and blrperiods.is_active = 1) ")
				...
				.append("            left join m_floating_rates_periods blrperiods on (blr.id = blrperiods.floating_rates_id and blrperiods.is_active = 1) ")
				.append("            where blr.is_base_lending_rate = 1 and blr.is_active = 1 ")
				.append("        ) as b ").append("        where b.from_date <= linkedrateperiods.from_date)) ")
		...
		public InterestRatePeriodData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return new InterestRatePeriodData(fromDate, interestRate, isDifferentialToBLR, blrFromDate,
					blrInterestRate);
		}
