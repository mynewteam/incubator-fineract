...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService
{
	...
	private void handleDataIntegrityIssues(final JsonCommand command, final Throwable realCause, final Exception dve)
	{
		...
		if (realCause.getMessage().contains("external_id"))
		{
			...
			throw new PlatformDataIntegrityException("error.msg.client.duplicate.externalId",
				"Client with externalId `" + externalId
					+ "` already exists",
		...
		} else if (realCause.getMessage().contains("account_no_UNIQUE"))
		{
			...
			throw new PlatformDataIntegrityException("error.msg.client.duplicate.accountNo",
				"Client with accountNo `" + accountNo
					+ "` already exists",
				"accountNo", accountNo);
	...
	public CommandProcessingResult createClient(final JsonCommand command)
	{
		...
		try
		{
			...
			{
				clientType = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(
	...
	/**
	 * This method extracts ClientNonPerson details from Client command and creates
	 * a new ClientNonPerson record
	 * 
	 * @param client
	 * @param command
	 */
	...
	public CommandProcessingResult assignClientStaff(final Long clientId, final JsonCommand command)
	{
		...
		if (staffId != null)
		{
			...
			/**
			 * TODO Vishwas: We maintain history of chage of loan officer w.r.t loan in a
			 * history table, should we do the same for a client? Especially useful when the
			 * change happens due to a transfer etc
			 **/
	...
	/*
	 * To become a part of a group, group may have set of criteria to be m et before
	 * client can become member of it.
	 */
	private void validateParentGroupRulesBeforeClientActivation(Client client)
	{
		...
		if (client.getGroups() != null && maxNumberOfClients != null)
		{
			...
			{
				/**
				 * Since this Client has not yet been associated with the group, reduce
				 * maxNumberOfClients by 1
				 **/
				final boolean validationsuccess = group.isGroupsClientCountWithinMaxRange(maxNumberOfClients - 1);
				if (!validationsuccess) { throw new GroupMemberCountNotInPermissibleRangeException(group.getId(),
					minNumberOfClients,
					maxNumberOfClients); }
			}
		}
