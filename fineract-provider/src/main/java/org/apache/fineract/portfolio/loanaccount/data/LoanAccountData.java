...
public class LoanAccountData
{
		...
		String dateFormat)
	{
		...
		return new LoanAccountData(loanTypeEnumOption, clientId, productId, loanOfficerId, submittedOnDate, fundId,
			...
			repaymentEvery, repaidEveryFrequencyEnums, loanTermFrequency, loanTermFrequencyTypeEnum,
			nominalInterestRate, expectedDisbursementDate,
			amortizationEnumOption, interestMethodEnum, interestCalculationPeriodTypeEnum, inArrearsTolerance,
			transactionProcessingStrategyId,
			graceOnPrincipalPayment, graceOnInterestPayment, graceOnInterestCharged, interestChargedFromDate,
			repaymentsStartingFromDate,
			rowIndex, externalId,loanaccountId, null, charges, linkAccountId, locale, dateFormat);
	}
		...
		String dateFormat)
	{
		...
		return new LoanAccountData(loanTypeEnumOption, groupIdforGroupLoan, productId, loanOfficerId, submittedOnDate,
			fundId,
			principal, numberOfRepayments,
			...
			interestChargedFromDate, repaymentsStartingFromDate, rowIndex, externalId,loanaccountId, null, null, linkAccountId,
			locale, dateFormat);
	}
	        ...
	        final Collection<CodeValueData> collateralStatusOption)
	{
		...
		final String loanaccountId=null;
		...
		return new LoanAccountData(id, accountNo, status, externalId,loanaccountId, clientId, clientAccountNo, clientName,
			clientOfficeId, group,
			loanType, loanProductId, loanProductName, loanProductDescription, isLoanProductLinkedToFloatingRate, fundId,
			fundName,
			loanPurposeId, loanPurposeName, loanOfficerId, loanOfficerName, currencyData, proposedPrincipal, principal,
			principal,
			totalOverpaid, inArrearsTolerance, termFrequency, termPeriodFrequencyType, numberOfRepayments,
			repaymentEvery,
			repaymentFrequencyType, repaymentFrequencyNthDayType, repaymentFrequencyDayOfWeekType,
			transactionProcessingStrategyId,
			transactionProcessingStrategyName, amortizationType, interestRatePerPeriod, interestRateFrequencyType,
			annualInterestRate,
			...
			originalSchedule,
			createStandingInstructionAtDisbursement, paidInAdvance, interestRatesPeriods, isVariableInstallmentsAllowed,
			minimumGap,
			maximumGap, subStatus, canUseForTopup, clientActiveLoanOptions, isTopup, closureLoanId,
		...
		final LocalDate expectedDisbursementDate)
	{
		...
		final Integer graceOnInterestCharged = null;
		final LocalDate interestChargedFromDate = null;
		final LoanApplicationTimelineData timeline = LoanApplicationTimelineData
			.templateDefault(expectedDisbursementDate);
		final LoanSummaryData summary = null;
		final BigDecimal feeChargesDueAtDisbursementCharged = null;
	...
	public static LoanAccountData populateClientDefaults(final LoanAccountData acc, final LoanAccountData clientAcc)
	{
		...
		return new LoanAccountData(acc.id, acc.accountNo, acc.status, acc.externalId,acc.loanaccountId, clientAcc.clientId,
			clientAcc.clientAccountNo,
			clientAcc.clientName, clientAcc.clientOfficeId, acc.group, acc.loanType, acc.loanProductId,
			acc.loanProductName,
			acc.loanProductDescription, acc.isLoanProductLinkedToFloatingRate, acc.fundId, acc.fundName,
			acc.loanPurposeId,
			acc.loanPurposeName, acc.loanOfficerId, acc.loanOfficerName, acc.currency, acc.proposedPrincipal,
			acc.principal,
			acc.approvedPrincipal, acc.totalOverpaid, acc.inArrearsTolerance, acc.termFrequency,
	...
	public static LoanAccountData populateGroupDefaults(final LoanAccountData acc, final LoanAccountData groupAcc)
	{
		...
		return new LoanAccountData(acc.id, acc.accountNo, acc.status, acc.externalId,acc.loanaccountId, acc.clientId, acc.clientAccountNo,
			acc.clientName,
			acc.clientOfficeId, groupAcc.group, acc.loanType, acc.loanProductId, acc.loanProductName,
			acc.loanProductDescription,
			acc.isLoanProductLinkedToFloatingRate, acc.fundId, acc.fundName, acc.loanPurposeId, acc.loanPurposeName,
			acc.loanOfficerId,
			acc.loanOfficerName, acc.currency, acc.proposedPrincipal, acc.principal, acc.approvedPrincipal,
			acc.totalOverpaid,
			acc.inArrearsTolerance, acc.termFrequency, acc.termPeriodFrequencyType, acc.numberOfRepayments,
			acc.repaymentEvery,
		...
		final Collection<LoanAccountSummaryData> clientActiveLoanOptions)
	{
		...
		if (product.useBorrowerCycle() && loanCycleNumber > 0)
		{
			...
			BigDecimal numberofRepaymentval = fetchLoanCycleDefaultValue(repaymentVariationsForBorrowerCycle,
				loanCycleNumber);
		...
		return new LoanAccountData(id, accountNo, status, externalId,loanaccountId, clientId, clientAccountNo, clientName,
			clientOfficeId, group,
			loanType, product.getId(), product.getName(), product.getDescription(),
			...
			product.getCurrency(), proposedPrincipal, principal, principal, totalOverpaid,
			...
			expectedFirstRepaymentOnDate, product.getGraceOnPrincipalPayment(),
			...
			memberVariations,
			product, inArrears, product.getGraceOnArrearsAgeing(), product.overdueFeeCharges(), isNPA,
			product.getDaysInMonthType(),
			product.getDaysInYearType(), product.isInterestRecalculationEnabled(),
			...
			product.getMaximumGapBetweenInstallments(), subStatus, canUseForTopup, clientActiveLoanOptions, isTopup,
			closureLoanId,
	...
	public static LoanAccountData populateLoanProductDefaults(final LoanAccountData acc, final LoanProductData product)
	{
		...
		return new LoanAccountData(acc.id, acc.accountNo, acc.status, acc.externalId,acc.loanaccountId, acc.clientId, acc.clientAccountNo,
			...
			product.getPrincipal(), product.getPrincipal(), acc.totalOverpaid, product.getInArrearsTolerance(),
			termFrequency,
			termPeriodFrequencyType, product.getNumberOfRepayments(), product.getRepaymentEvery(),
			...
			product.getAllowPartialPeriodInterestCalcualtion(), acc.expectedFirstRepaymentOnDate,
			...
			acc.memberVariations, product, acc.inArrears, product.getGraceOnArrearsAgeing(),
			product.overdueFeeCharges(), acc.isNPA,
		...
		final Collection<LoanAccountSummaryData> clientActiveLoanOptions)
	{
		...
		return new LoanAccountData(acc.id, acc.accountNo, acc.status, acc.externalId,acc.loanaccountId, acc.clientId, acc.clientAccountNo,
			acc.clientName,
			acc.clientOfficeId, acc.group, acc.loanType, acc.loanProductId, acc.loanProductName,
			acc.loanProductDescription,
			acc.isLoanProductLinkedToFloatingRate, acc.fundId, acc.fundName, acc.loanPurposeId, acc.loanPurposeName,
			acc.loanOfficerId,
			acc.loanOfficerName, acc.currency, acc.proposedPrincipal, acc.principal, acc.approvedPrincipal,
			...
			repaymentFrequencyDayOfWeekTypeOptions, transactionProcessingStrategyOptions,
		...
		for (Map.Entry<Long, Integer> mapEntry : memberLoanCycle.entrySet())
		{
			...
			if (acc.product.useBorrowerCycle() && loanCycleNumber != null && loanCycleNumber > 0)
			{
				...
				Collection<LoanProductBorrowerCycleVariationData> repaymentVariationsForBorrowerCycle = acc.product
					.getNumberOfRepaymentVariationsForBorrowerCycle();
				principal = fetchLoanCycleDefaultValue(principalVariationsForBorrowerCycle, loanCycleNumber);
				interestRatePerPeriod = fetchLoanCycleDefaultValue(interestForVariationsForBorrowerCycle,
					loanCycleNumber);
				BigDecimal numberofRepaymentval = fetchLoanCycleDefaultValue(repaymentVariationsForBorrowerCycle,
					loanCycleNumber);
				if (numberofRepaymentval != null)
			...
			LoanBorrowerCycleData borrowerCycleData = new LoanBorrowerCycleData(principal, interestRatePerPeriod,
				numberOfRepayments,
				termFrequency);
	...
	public static LoanAccountData withLoanCalendarData(final LoanAccountData acc, final CalendarData calendarData)
	{
		return new LoanAccountData(acc.id, acc.accountNo, acc.status, acc.externalId,acc.loanaccountId, acc.clientId, acc.clientAccountNo,
			...
			acc.overdueCharges,
			acc.isNPA, acc.daysInMonthType, acc.daysInYearType, acc.isInterestRecalculationEnabled,
	...
	{
		return this.timeline.repaymentScheduleRelatedData(this.currency, this.principal, this.approvedPrincipal,
