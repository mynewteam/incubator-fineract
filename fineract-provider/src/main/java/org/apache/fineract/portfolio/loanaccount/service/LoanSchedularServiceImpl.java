		...
		if (!overdueLoanScheduledInstallments.isEmpty())
		{
			...
			for (final Long loanId : overdueScheduleData.keySet())
			{
				...
				} catch (final PlatformApiDataValidationException e)
				{
					...
					{
						logger
							.error("Apply Charges due for overdue loans failed for account:" + loanId + " with message "
								+ error.getDeveloperMessage());
						sb.append("Apply Charges due for overdue loans failed for account:").append(loanId)
							.append(" with message ")
				...
				} catch (Exception e)
				{
					...
					logger.error(
						"Apply Charges due for overdue loans failed for account:" + loanId + " with message "
				...
				while (numberOfRetries <= maxNumberOfRetries)
				{
					...
					} catch (Exception e)
					{
						...
						logger.error("Interest recalculation for loans failed for account:" + loanId + " with message "
							+ realCause.getMessage());
						sb.append("Interest recalculation for loans failed for account:").append(loanId)
							.append(" with message ")
							.append(realCause.getMessage());
						numberOfRetries = maxNumberOfRetries + 1;
					}
