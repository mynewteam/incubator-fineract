...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator
{
		...
		final LoanScheduleParams loanScheduleParams)
	{
		...
		LoanTermVariationsData lastDueDateVariation = loanApplicationTerms.getLoanTermVariations()
			.fetchLoanTermDueDateVariationsData(
				loanEndDate);
		...
		if (!scheduleParams.isPartialUpdate())
		{
			...
			if (loanApplicationTerms.isMultiDisburseLoan())
			{
				...
				// tranche
				// details to map
				BigDecimal disburseAmt = getDisbursementAmount(
					loanApplicationTerms,
			...
			&& currentDate.isAfter(scheduleParams.getPeriodStartDate()))
		{
			...
			logger.info(
				"outstaning :" + scheduleParams.getOutstandingBalance() + ": interest" + totalInterest.toString());
		...
		ScheduleCurrentPeriodParams currentPeriodParams)
	{
		...
		currentPeriodParams
			.setFeeChargesForInstallment(cumulativeFeeChargesDueWithin(scheduleParams.getPeriodStartDate(),
		...
		currentPeriodParams
			.setPenaltyChargesForInstallment(cumulativePenaltyChargesDueWithin(scheduleParams.getPeriodStartDate(),
				scheduledDueDate, loanCharges, currency, principalInterest, scheduleParams.getPrincipalToBeScheduled(),
				scheduleParams.getTotalCumulativeInterest(), true));
		...
		Set<LoanCharge> loanCharges)
	{
		...
		oustanding = oustanding.minus(cumulativeFeeChargesDueWithin(transactionDate,
			scheduledDueDate, loanCharges, totalInterestChargedForFullLoanTerm.getCurrency(), tempPrincipalInterest,
							...
							.isInterestFirstRepaymentScheduleTransactionProcessor())
					{
							...
							|| scheduleParams.getInstalmentNumber() == 1)
						{
							...
							if (daysInPeriodApplicable > 0)
							{
								...
								scheduleParams
									.setTotalOutstandingInterestPaymentDueToGrace(principalInterestForThisPeriod
		...
		do
		{
			...
			if (!outstanding.isZero())
			{
				...
				if (loanApplicationTerms.getInterestRecalculationCompoundingMethod().isCompoundingEnabled())
				{
					...
					{
						params.setCompoundedInLastInstallment(uncompoundedFromLastInstallment);// uncompounded in last
																								// installment
		...
		)
	{
		if (loanCharge.getChargeCalculation().isPercentageBased())
		{
			...
			} else if (loanCharge.getChargeCalculation().isPercentageOfInterest())
			{
			 ...
			 else if (loanCharge.getChargeCalculation().isPercentageOfOutstandingAmount()) //if charge type is percentage of Outstanding amount
				{
...
//            loanChargeAmt = .getPrincipalOutstanding().getAmount();
				...
				if (installment.getDueDate().isAfter(lastInstallmentDate))
				{
					...
					do
					{
						...
						lastInstallmentDate = this.scheduledDateGenerator
							.adjustRepaymentDate(actualRepaymentDate, loanApplicationTerms,
								holidayDetailDTO)
		...
		final LoanScheduleModelPeriod scheduledLoanInstallment)
	{
		...
		if (scheduledLoanInstallment.isRepaymentPeriod())
		{
			...
			installment = new LoanRepaymentScheduleInstallment(
				null,
