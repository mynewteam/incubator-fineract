...
public class LoanWritePlatformServiceJpaRepositoryImpl implements LoanWritePlatformService
{
		...
		if (canDisburse)
		{
			...
			} else
			{
				...
				LoanTransaction disbursementTransaction = LoanTransaction.disbursement(loan.getOffice(),
					amountToDisburse, paymentDetail,
	...
	private void createStandingInstruction(Loan loan)
	{
		...
		{
			AccountAssociations accountAssociations = this.accountAssociationRepository.findByLoanIdAndType(
				loan.getId(),
	...
	private void saveAndFlushLoanWithDataIntegrityViolationChecks(final Loan loan)
	{
		...
		} catch (final DataIntegrityViolationException e)
		{
			...
			if (!dataValidationErrors
				.isEmpty()) { throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist",
		...
		Boolean isAccountTransfer)
	{
		...
		for (int i = 0; i < disbursalCommand.length; i++)
		{
			...
			for (final Map.Entry<Long, BigDecimal> entrySet : disBuLoanCharges.entrySet())
			{
				...
				final AccountTransferDTO accountTransferDTO = new AccountTransferDTO(actualDisbursementDate,
					entrySet.getValue(),
					PortfolioAccountType.SAVINGS, PortfolioAccountType.LOAN, savingAccountData.accountId(),
					loan.getId(),
					"Loan Charge Payment", locale, fmt, null, null,
		...
		for (final SingleRepaymentCommand singleLoanRepaymentCommand : repaymentCommand)
		{
			if (singleLoanRepaymentCommand != null)
			{
				Loan loan = this.loanRepositoryWrapper
					.findOneWithNotFoundDetection(singleLoanRepaymentCommand.getLoanId());
		...
		for (final SingleRepaymentCommand singleLoanRepaymentCommand : repaymentCommand)
		{
			if (singleLoanRepaymentCommand != null)
			{
				...
				LoanTransaction loanTransaction = this.loanAccountDomainService.makeRepayment(loan,
					...
					bulkRepaymentCommand.getTransactionDate(), singleLoanRepaymentCommand.getTransactionAmount(),
					paymentDetail,
		...
		final JsonCommand command)
	{
		 ...
		 * HibernateOptimisticLockingFailureException, looping and saving for the time
		 * being, not a major issue for now as this loop is entered only in edge cases
		 * (when a adjustment is made before the latest payment recorded against the
		 * loan)
		 ***/
		saveAndFlushLoanWithDataIntegrityViolationChecks(loan);
		if (changedTransactionDetail != null)
		...
		if (StringUtils.isNotBlank(noteText))
		{
			...
			Note note = null;
			/**
			 * If a new transaction is not created, associate note with the transaction to
			 * be adjusted
			 **/
			if (newTransactionDetail.isGreaterThanZero(loan.getPrincpal().getCurrency()))
			{
	...
	public CommandProcessingResult waiveInterestOnLoan(final Long loanId, final JsonCommand command)
	{
		 ...
		 * HibernateOptimisticLockingFailureException, looping and saving for the time
		 * being, not a major issue for now as this loop is entered only in edge cases
		 * (when a waiver is made before the latest payment recorded against the loan)
		 ***/
		saveAndFlushLoanWithDataIntegrityViolationChecks(loan);
		if (changedTransactionDetail != null)
		{
	...
	public CommandProcessingResult writeOff(final Long loanId, final JsonCommand command)
	{
		...
		this.businessEventNotifierService.notifyBusinessEventToBeExecuted(BUSINESS_EVENTS.LOAN_WRITTEN_OFF,
			constructEntityMap(BUSINESS_ENTITY.LOAN, loan));
		entityDatatableChecksWritePlatformService.runTheCheckForProduct(loanId, EntityTables.LOAN.getName(),
			StatusEnum.WRITE_OFF.getCode().longValue(), EntityTables.LOAN.getForeignKeyColumnNameOnDatatable(),
			loan.productId());
	...
	public CommandProcessingResult addLoanCharge(final Long loanId, final JsonCommand command)
	{
		...
		if (chargeDefinition.isPercentageOfDisbursementAmount())
		{
			...
			for (LoanDisbursementDetails disbursementDetail : loanDisburseDetails)
			{
				if (disbursementDetail.actualDisbursementDate() == null)
				{
					loanCharge = LoanCharge.createNewWithoutLoan(
						chargeDefinition, 
	...
	private boolean addCharge(final Loan loan, final Charge chargeDefinition, final LoanCharge loanCharge)
	{
		...
		if (loanCharge.getChargePaymentMode().isPaymentModeAccountTransfer())
		{
			final PortfolioAccountData portfolioAccountData = this.accountAssociationsReadPlatformService
				.retriveLoanLinkedAssociation(loan
					.getId());
		...
		final JsonCommand command)
	{
		...
		// active
		if (!loan.status()
			.isActive()) { throw new LoanChargeCannotBeWaivedException(
				LOAN_CHARGE_CANNOT_BE_WAIVED_REASON.LOAN_INACTIVE,
				loanCharge.getId()); }
		...
		if (loanCharge.isWaived())
		{
			...
			.isPaid()) { throw new LoanChargeCannotBeWaivedException(LOAN_CHARGE_CANNOT_BE_WAIVED_REASON.ALREADY_PAID,
				loanCharge.getId()); }
		...
		final LoanTransaction waiveTransaction = loan.waiveLoanCharge(loanCharge, defaultLoanLifecycleStateMachine(),
			changes,
			existingTransactionIds, existingReversedTransactionIds, loanInstallmentNumber, scheduleGeneratorDTO,
			accruedCharge,
			currentUser);
		...
		final boolean isChargeIdIncludedInJson)
	{
		...
		// active
		if (!loan.status()
			.isActive()) { throw new LoanChargeCannotBePayedException(LOAN_CHARGE_CANNOT_BE_PAYED_REASON.LOAN_INACTIVE,
				loanCharge.getId()); }
		...
		if (!loanCharge.getChargePaymentMode()
			.isPaymentModeAccountTransfer()) { throw new LoanChargeCannotBePayedException(
		...
		final AccountTransferDTO accountTransferDTO = new AccountTransferDTO(transactionDate, amount,
			...
			PortfolioAccountType.LOAN, portfolioAccountData.accountId(), loanId, "Loan Charge Payment", locale, fmt,
			null, null,
	...
	public void disburseLoanToLoan(final Loan loan, final JsonCommand command, final BigDecimal amount)
	{
		...
		final AccountTransferDTO accountTransferDTO = new AccountTransferDTO(transactionDate, amount,
			...
			"Loan Topup", locale, fmt, LoanTransactionType.DISBURSEMENT.getValue(),
		...
		final PaymentDetail paymentDetail)
	{
		...
		final AccountTransferDTO accountTransferDTO = new AccountTransferDTO(transactionDate, amount.getAmount(),
			...
			"Loan Disbursement", locale, fmt, paymentDetail, LoanTransactionType.DISBURSEMENT.getValue(), null, null,
			null,
						...
						if (!installmentChargeData.getDueDate().isAfter(new LocalDate()))
						{
							if (portfolioAccountData == null)
							{
								portfolioAccountData = this.accountAssociationsReadPlatformService
									.retriveLoanLinkedAssociation(chargeData
										.getLoanId());
							}
							final SavingsAccount fromSavingsAccount = null;
							final boolean isExceptionForBalanceCheck = false;
							final AccountTransferDTO accountTransferDTO = new AccountTransferDTO(new LocalDate(),
								...
								portfolioAccountData.accountId(), chargeData.getLoanId(), "Loan Charge Payment", null,
								null, null,
	...
	private void transferFeeCharge(final StringBuilder sb, final AccountTransferDTO accountTransferDTO)
	{
		...
		} catch (final InsufficientAccountBalanceException e)
		{
			sb.append("InsufficientAccountBalance Exception while paying charge ")
				.append(accountTransferDTO.getChargeId())
	...
	public CommandProcessingResult loanReassignment(final Long loanId, final JsonCommand command)
	{
		...
		this.businessEventNotifierService.notifyBusinessEventToBeExecuted(BUSINESS_EVENTS.LOAN_REASSIGN_OFFICER,
			constructEntityMap(BUSINESS_ENTITY.LOAN, loan));
		if (!loan
			.hasLoanOfficer(fromLoanOfficer)) { throw new LoanOfficerAssignmentException(loanId, fromLoanOfficerId); }
	...
	public CommandProcessingResult bulkLoanReassignment(final JsonCommand command)
	{
		...
		for (final String loanIdString : loanIds)
		{
			...
			if (!loan.hasLoanOfficer(
				fromLoanOfficer)) { throw new LoanOfficerAssignmentException(loanId, fromLoanOfficerId); }
		...
		final LocalDate newMeetingDate)
	{
		...
		final Collection<Integer> loanStatuses = new ArrayList<>(
			Arrays.asList(LoanStatus.SUBMITTED_AND_PENDING_APPROVAL.getValue(),
		...
		final List<Loan> loans = this.loanRepositoryWrapper.findByIdsAndLoanStatusAndLoanType(loanIds, loanStatuses,
			loanTypes);
	...
	private void removeLoanCycle(final Loan loan)
	{
		...
		if (loan.isGroupLoan())
		{
			...
			{
				loansToUpdate = this.loanRepositoryWrapper.getGroupLoansToUpdateLoanCounter(
					loan.getCurrentLoanCounter(), loan.getGroupId(),
					AccountType.GROUP.getValue());
			} else
			{
				loansToUpdate = this.loanRepositoryWrapper.getGroupLoansToUpdateLoanProductCounter(
					loan.getLoanProductLoanCounter(),
		...
		} else
		{
			...
			{
				loansToUpdate = this.loanRepositoryWrapper.getClientLoansToUpdateLoanProductCounter(
					loan.getLoanProductLoanCounter(),
					loan.getClientId());
			}
	...
	private Integer getNewGroupLoanCounter(final Loan loan)
	{
		...
		Integer maxClientLoanCounter = this.loanRepositoryWrapper.getMaxGroupLoanCounter(loan.getGroupId(),
			AccountType.GROUP.getValue());
	...
	private Integer getNewGroupLoanProductCounter(final Loan loan)
	{
		...
		Integer maxLoanProductLoanCounter = this.loanRepositoryWrapper.getMaxGroupLoanProductCounter(
	...
	private Integer getNewClientOrJLGLoanProductCounter(final Loan loan)
	{
		...
		Integer maxLoanProductLoanCounter = this.loanRepositoryWrapper.getMaxClientOrJLGLoanProductCounter(
			loan.loanProduct().getId(),
			loan.getClientId());
		if (maxLoanProductLoanCounter == null)
		{
	...
	private void checkForProductMixRestrictions(final Loan loan)
	{
		...
		if (loan.isGroupLoan())
		{
			activeLoansLoanProductIds = this.loanRepositoryWrapper.findActiveLoansLoanProductIdsByGroup(
				loan.getGroupId(),
				LoanStatus.ACTIVE.getValue());
		} else
		{
			activeLoansLoanProductIds = this.loanRepositoryWrapper.findActiveLoansLoanProductIdsByClient(
				loan.getClientId(),
				LoanStatus.ACTIVE.getValue());
		}
		...
		final String productName)
	{
		...
		if (!CollectionUtils.isEmpty(activeLoansLoanProductIds))
		{
			...
			{
				if (activeLoansLoanProductIds
					.contains(restrictedProduct.getId())) { throw new LoanDisbursalException(productName,
		...
		if (lastChargeDate != null)
		{
			...
			if (lastChargeDate.isAfter(lastInstallment.getDueDate()))
			{
				...
				LoanRepaymentScheduleInstallment newEntry = new LoanRepaymentScheduleInstallment(loan,
					installments.size() + 1,
					lastInstallment.getDueDate(), lastChargeDate, principal, outstanding, interest, feeCharges,
					penaltyCharges,
		...
		final List<Long> existingReversedTransactionIds)
	{
		...
		Collection<Integer> frequencyNumbers = loanChargeReadPlatformService
			.retrieveOverdueInstallmentChargeFrequencyNumber(loanId,
	...
	{
		if (!loan.getExpectedDisbursedOnLocalDate()
			.equals(actualDisbursementDate)) { throw new DateMismatchException(actualDisbursementDate,
