...
public class AppuserLoansMapperReadServiceImpl implements AppuserLoansMapperReadService {
	...
	public Boolean isLoanMappedToUser(Long loanId, Long appUserId) {
		return this.jdbcTemplate.queryForObject("select case when (count(*) > 0) then true else false end "
				+ " from m_selfservice_user_client_mapping  m " + " left join m_loan  l on l.client_id = m.client_id "
				+ " where l.id = ? and m.appuser_id = ? ", new Object[] { loanId, appUserId }, Boolean.class);
	}
