...
public class Loan extends AbstractPersistableCustom<Long> {
	...
	@Column(name = "collateral_numid")
	private String collateralnumid;
	...
	public static Loan newIndividualLoanApplication(final String accountNo, final Client client, final Integer loanType,
			final String loanAccountId, final String referenceid, final String collateralnumid,
			...
			final Set<LoanCollateral> collateral, final Set<LandCollateral> landCollateral,
		...
		return new Loan(accountNo, client, group, loanType, fund, officer, loanPurpose, transactionProcessingStrategy,
				loanProduct, loanRepaymentScheduleDetail, status, loanCharges, collateral, landCollateral,
	...
	public String getCollateralnumid() {
		return collateralnumid;
	}

	public void setCollateralnumid(String collateralnumid) {
		this.collateralnumid = collateralnumid;
	}

	public static Loan newIndividualLoanApplicationFromGroup(final String accountNo, final Client client,
			final Group group, final Integer loanType, final String loanAccountId, final String referenceid,
			final String collateralnumid, final LoanProduct loanProduct, final Fund fund, final Staff officer,
	...
	private Loan(
			...
			final String accountNo, final Client client, final Group group, final Integer loanType, final Fund fund,
			...
			final Set<LandCollateral> landCollateral, final Boolean syncDisbursementWithMeeting,
			final BigDecimal fixedEmiAmount, final List<LoanDisbursementDetails> disbursementDetails,
			final BigDecimal maxOutstandingLoanBalance, final Boolean createStandingInstructionAtDisbursement,
			final Boolean isFloatingInterestRate, final BigDecimal interestRateDifferential) {
		...
		/*
		 * During loan origination stage and before loan is approved principal_amount,
		 * approved_principal and principal_amount_demanded will same amount and that
		 * amount is same as applicant loan demanded amount.
		 */
	...
	public void addLoanCharge(final LoanCharge loanCharge) {
		...
		logger.info("Sothea Working On: public void addLoanCharge(final LoanCharge loanCharge: amount " + amount);
	...
	/**
	 * Creates a loanTransaction for "Apply Charge Event" with transaction date set
	 * to "suppliedTransactionDate". The newly created transaction is also added to
	 * the Loan on which this method is called.
	 * 
	 * If "suppliedTransactionDate" is not passed Id, the transaction date is set to
	 * the loans due date if the due date is lesser than todays date. If not, the
	 * transaction date is set to todays date
	 * 
	 * @param loanCharge
	...
	public void removeLoanCharge(final LoanCharge loanCharge) {
		...
		if (!loanCharge.isDueAtDisbursement() && loanCharge.isPaidOrPartiallyPaid(loanCurrency())) {
			/****
			 * TODO Vishwas Currently we do not allow removing a loan charge after a loan is
			 * approved (hence there is no need to adjust any loan transactions).
			 * 
			 * Consider removing this block of code or logically completing it for the
			 * future by getting the list of affected Transactions
			 ***/
			final List<LoanTransaction> allNonContraTransactionsPostDisbursement = retreiveListOfTransactionsPostDisbursement();
	...
	public Map<String, Object> updateLoanCharge(final LoanCharge loanCharge, final JsonCommand command) {
		...
		if (!loanCharge.isDueAtDisbursement()) {
			/****
			 * TODO Vishwas Currently we do not allow waiving updating loan charge after a
			 * loan is approved (hence there is no need to adjust any loan transactions).
			 * 
			 * Consider removing this block of code or logically completing it for the
			 * future by getting the list of affected Transactions
			 ***/
...
//        logger.info("Sothea Working On  Class Loan::calculateAmountPercentageAppliedTo(final LoanCharge loanCharge) " + loanCharge.getChargeCalculation());
		...
		case PERCENT_OF_DISBURSEMENT_AMOUNT:
			...
			break;
		case PERCENT_OF_OUSTANDING_AMOUNT:
			logger.info("Sothea Working On  Class Loan::case PERCENT_OF_OUSTANDING_AMOUNT:  "
	...
	public BigDecimal getTotalInterest() {
		return this.loanSummaryWrapper.calculateTotalInterestCharged(getRepaymentScheduleInstallments(), getCurrency())
				.getAmount();
	}
	...
	private Money calculateInstallmentChargeAmount(final ChargeCalculationType calculationType,
		...
		case PERCENT_OF_INTEREST:
			...
			break;
		case PERCENT_OF_DISBURSEMENT_AMOUNT:
			percentOf = installment.getFeeChargesOutstanding(getCurrency());
			break;
		case PERCENT_OF_OUSTANDING_AMOUNT:
			logger.info("Sothea Working On: percentOf PERCENT_OF_OUSTANDING_AMOUNT  "
					+ installment.getOutstanding(getCurrency()));
			percentOf = installment.getOutstanding(getCurrency());
			break;
		default:
			break;
		}
	...
	public LoanTransaction waiveLoanCharge(final LoanCharge loanCharge,
		...
		if (!loanCharge.isDueAtDisbursement() && loanCharge.isPaidOrPartiallyPaid(loanCurrency())) {
			/****
			 * TODO Vishwas Currently we do not allow waiving fully paid loan charge and
			 * waiving partially paid loan charges only waives the remaining amount.
			 * 
			 * Consider removing this block of code or logically completing it for the
			 * future by getting the list of affected Transactions
			 ***/
		...
		for (final LoanScheduleModelPeriod scheduledLoanInstallment : modifiedLoanSchedule.getPeriods()) {
			...
			if (scheduledLoanInstallment.isRepaymentPeriod()) {
				final LoanRepaymentScheduleInstallment installment = new LoanRepaymentScheduleInstallment(this,
						...
						scheduledLoanInstallment.outstandingDue(), scheduledLoanInstallment.interestDue(),
	...
	private void updateLoanSummaryDerivedFields() {
		...
		if (isNotDisbursed()) {
			...
			this.summary.updateSummary(loanCurrency(), principal, getRepaymentScheduleInstallments(),
	...
	public Map<String, Object> loanApplicationModification(final JsonCommand command,
			...
			final Set<LandCollateral> possiblyModifedLandCollateralItems, final AprCalculator aprCalculator,
		...
		final String loanAccountId = "loanAccountId";
		if (command.isChangeInStringParameterNamed(loanAccountId, this.loanaccountId)) {
			final String newValue = command.stringValueOfParameterNamed(loanAccountId);
			actualChanges.put(loanAccountId, newValue);
			this.loanaccountId = StringUtils.defaultIfEmpty(newValue, null);
		}

		final String referenceid = "referenceid";
		if (command.isChangeInStringParameterNamed(referenceid, this.referenceid)) {
			final String newValue = command.stringValueOfParameterNamed(referenceid);
			actualChanges.put(referenceid, newValue);
			this.referenceid = StringUtils.defaultIfEmpty(newValue, null);
		}

		final String collateralnumid = "collateralnumid";
		if (command.isChangeInStringParameterNamed(collateralnumid, this.collateralnumid)) {
			final String newValue = command.stringValueOfParameterNamed(collateralnumid);
			actualChanges.put(collateralnumid, newValue);
			this.collateralnumid = StringUtils.defaultIfEmpty(newValue, null);
		}
		...
		final String loanaccountidParamName = "loanaccountId";
		if (command.isChangeInStringParameterNamed(loanaccountidParamName, this.loanaccountId)) {
			final String newValue = command.stringValueOfParameterNamed(loanaccountidParamName);
			actualChanges.put(loanaccountidParamName, newValue);
			this.loanaccountId = StringUtils.defaultIfEmpty(newValue, null);
		}

		final String referenceidParamName = "referenceid";
		if (command.isChangeInStringParameterNamed(referenceidParamName, this.referenceid)) {
			final String newValue = command.stringValueOfParameterNamed(referenceidParamName);
			actualChanges.put(referenceidParamName, newValue);
			this.referenceid = StringUtils.defaultIfEmpty(newValue, null);
		}

		final String collateralnumidParamName = "collateralnumid";
		if (command.isChangeInStringParameterNamed(collateralnumidParamName, this.collateralnumid)) {
			final String newValue = command.stringValueOfParameterNamed(collateralnumidParamName);
			actualChanges.put(collateralnumidParamName, newValue);
			this.collateralnumid = StringUtils.defaultIfEmpty(newValue, null);
		}
		...
		if (command.parameterExists(landCollateralParamName)) {
			if (!possiblyModifedLandCollateralItems.equals(this.landCollateral)) {
				actualChanges.put(landCollateralParamName,
						listOfLandCollateralData(possiblyModifedLandCollateralItems));
			}
		}
...
//		}
	...
	public void loanApplicationSubmittal(final AppUser currentUser, final LoanScheduleModel loanSchedule,
			...
			final LocalDate submittedOn, final String externalId, final String loanaccountId, final String referenceid,
			final String collateralnumid, final boolean allowTransactionsOnHoliday, final List<Holiday> holidays,
	...
	public Map<String, Object> loanApplicationApproval(final AppUser currentUser, final JsonCommand command,
		 ...
		 * loanLifecycleStateMachine.transition is doing it's responsibility properly.
		 * Better implementation approach is, if code passes invalid combination of
		 * states (fromState and toState), state machine should return invalidate state
		 * and below if condition should check for not equal to invalidateState, instead
		 * of check new value is same as present value.
		 */
		...
		if (!newStatusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
			...
			if (approvedLoanAmount != null) {
				...
				if (approvedLoanAmount.compareTo(this.proposedPrincipal) == -1) {
					...
					/*
					 * All the calculations are done based on the principal amount, so it is
					 * necessary to set principal amount to approved amount
					 */
	...
	public ChangedTransactionDetail disburse(final AppUser currentUser, final JsonCommand command,
		...
		/**
		 * Add an interest applied transaction of the interest is accrued upfront (Up
		 * front accrual), no accounting or cash based accounting is selected
		 **/
		...
		if (loanApplicationTerms.isEqualAmortization()) {
			if (loanApplicationTerms.getInterestMethod().isDecliningBalnce()) {
				final LoanScheduleGenerator decliningLoanScheduleGenerator = scheduleGeneratorDTO
						.getLoanScheduleFactory().create(InterestMethod.DECLINING_BALANCE);
				...
				LoanScheduleModel loanSchedule = decliningLoanScheduleGenerator.generate(mc, loanApplicationTerms,
						loanCharges, scheduleGeneratorDTO.getHolidayDetailDTO());
	...
	private void handleDisbursementTransaction(final LocalDate disbursedOn, final LocalDateTime createdDate,
		...
		for (final LoanCharge charge : charges()) {
			...
			if ((charge.getCharge().getChargeTimeType() == ChargeTimeType.DISBURSEMENT.getValue()
				...
				/**
				 * create a Charge applied transaction if Up front Accrual, None or Cash based
	...
	private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(final LoanTransaction loanTransaction,
		...
		if (reprocess) {
			...
			/***
			 * Commented since throwing exception if external id present for one of the
			 * transactions. for this need to save the reversed transactions first and then
			 * new transactions.
			 */
			this.loanTransactions.addAll(changedTransactionDetail.getNewTransactionMappings().values());
		}
	...
	private void handleLoanRepaymentInFull(final LocalDate transactionDate,
		...
		for (final LoanCharge loanCharge : this.charges) {
			if (loanCharge.isActive() && loanCharge.amount().compareTo(BigDecimal.ZERO) == 1
	...
	public ChangedTransactionDetail closeAsWrittenOff(final JsonCommand command,
		...
		if (!statusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
			...
			final String tnxloanaccountId = command.stringValueOfParameterNamedAllowingNull("loanaccountId");
	 ...
	 * support easier transition to microweb platform.
	 */
	...
	public void updateLoanRepaymentScheduleDates(final LocalDate meetingStartDate, final String recuringRule,
		...
		/*
		 * meetingStartDate is used as seedDate Capture the seedDate from user and use
		 * the seedDate as meetingStart date
		 */
	...
	public ChangedTransactionDetail recalculateScheduleFromLastTransaction(final ScheduleGeneratorDTO generatorDTO,
		 ...
		 * (LoanTransaction loanTransaction : loanTransactions) { if (recalculateFrom ==
		 * null || loanTransaction.getTransactionDate().isAfter(recalculateFrom)) {
	...
	public LoanRepaymentScheduleInstallment fetchPrepaymentDetail(final ScheduleGeneratorDTO scheduleGeneratorDTO,
		...
		if (this.loanRepaymentScheduleDetail.isInterestRecalculationEnabled()) {
			...
			installment = loanScheduleGenerator.calculatePrepaymentAmount(getCurrency(), onDate, loanApplicationTerms,
					mc, this, scheduleGeneratorDTO.getHolidayDetailDTO(), loanRepaymentScheduleTransactionProcessor);
		} else {
	...
	private LoanRepaymentScheduleInstallment getTotalOutstandingOnLoan() {
		...
		return new LoanRepaymentScheduleInstallment(null, 0, LocalDate.now(), LocalDate.now(),
				totalPrincipal.getAmount(), null, totalInterest.getAmount(), feeCharges.getAmount(),
	...
	public List<DisbursementData> getDisbursmentData() {
		...
		while (iterator.hasNext()) {
			...
			disbursementData.add(new DisbursementData(loanDisbursementDetails.getId(), expectedDisbursementDate,
					actualDisbursementDate, loanDisbursementDetails.principal(), null, null, waivedChargeAmount));
		}
	 ...
	 * @param numberofdays
	 * @param holidayDetailDTO            Used for accessing the loan's calendar
	 *                                    object
	...
	public LoanRepaymentScheduleInstallment fetchLoanForeclosureDetail(final LocalDate closureDate) {
		...
		final Set<LoanInterestRecalcualtionAdditionalDetails> compoundingDetails = null;
		final LocalDate currentDate = DateUtils.getLocalDateOfTenant();
		return new LoanRepaymentScheduleInstallment(null, 0, currentDate, currentDate, totalPrincipal.getAmount(), null,
	...
	public Money[] retriveIncomeOutstandingTillDate(final LocalDate paymentDate) {
		...
		for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
			if (!installment.getDueDate().isAfter(paymentDate)) {
				...
				Money[] balancesForCurrentPeroid = fetchInterestFeeAndPenaltyTillDate(paymentDate, currency,
						installment);
	...
	private Money[] fetchInterestFeeAndPenaltyTillDate(final LocalDate paymentDate, final MonetaryCurrency currency,
		...
		interestForCurrentPeriod = Money.of(getCurrency(), BigDecimal.valueOf(calculateInterestForDays(totalPeriodDays,
	...
	public ChangedTransactionDetail handleForeClosureTransactions(final LoanTransaction repaymentTransaction,
		...
		return handleRepaymentOrRecoveryOrWaiverTransaction(repaymentTransaction, loanLifecycleStateMachine, null,
	...
	public void updateInstallmentsPostDate(LocalDate transactionDate) {
		...
		LoanRepaymentScheduleInstallment newInstallment = new LoanRepaymentScheduleInstallment(null,
				newInstallments.size() + 1, installmentStartDate, transactionDate, totalPrincipal.getAmount(), null,
