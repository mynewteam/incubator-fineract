...
//        logger.info("Sothea Working On  Class Loan::calculateAmountPercentageAppliedTo(final LoanCharge loanCharge) " + loanCharge.getChargeCalculation());
		...
		final AppUser currentUser)
	{
		...
		final LoanTransaction waiveLoanChargeTransaction = LoanTransaction.waiveLoanCharge(this, getOffice(),
			amountWaived, transactionDate,
			feeChargesWaived, penaltyChargesWaived, unrecognizedIncome, DateUtils.getLocalDateTimeOfTenant(),
			currentUser);
			...
			&& jsonObject.get(LoanApiConstants.disbursementDateParameterName).isJsonPrimitive())
		{
			...
			if (StringUtils.isNotBlank(valueAsString))
			{
				LocalDate date = JsonParserHelper.convertFrom(valueAsString,
					LoanApiConstants.disbursementDateParameterName, dateFormat,
					locale);
				if (date != null)
				{
			...
			&& emiAmount.compareTo(retriveLastEmiAmount()) != 0)
		{
			if (this.loanProduct.isMultiDisburseLoan())
			{
				...
				LoanTermVariations loanVariationTerms = new LoanTermVariations(
					...
					effectiveDateFrom, emiAmount, dateValue, isSpecificToInstallment, this,
		...
		switch (event)
		{
			...
			case LOAN_REJECTED:
				if (!isSubmittedAndPendingApproval())
				{
					...
					final ApiParameterError error = ApiParameterError
						.generalError("error.msg.loan.reject.account.is.not.submitted.pending.approval.state",
							defaultUserMessage);
					dataValidationErrors.add(error);
				}
				break;
			case LOAN_WITHDRAWN:
				if (!isSubmittedAndPendingApproval())
				{
					...
					final ApiParameterError error = ApiParameterError
						.generalError("error.msg.loan.withdrawn.account.is.not.submitted.pending.approval.state",
							defaultUserMessage);
					dataValidationErrors.add(error);
				}
				break;
		...
		LoanTransaction existingAccrualTransaction)
	{
		...
		if (existingIncomeTransaction == null)
		{
			LoanTransaction transaction = LoanTransaction.incomePosting(this, this.getOffice(),
				compoundingDetail.getEffectiveDate().toDate(), compoundingDetail.getAmount(), interest, fee, penalties,
				currentUser);
	...
	public LoanApplicationTerms constructLoanApplicationTerms(final ScheduleGeneratorDTO scheduleGeneratorDTO)
	{
		...
		final LoanApplicationTerms loanApplicationTerms = LoanApplicationTerms.assembleFrom(
			...
			getExpectedFirstRepaymentOnDate(), scheduleGeneratorDTO.getCalculatedRepaymentsStartingFromDate(),
	...
	public List<DisbursementData> getDisbursmentData()
	{
		...
		while (iterator.hasNext())
		{
			...
			disbursementData.add(
				new DisbursementData(loanDisbursementDetails.getId(), expectedDisbursementDate, actualDisbursementDate,
	...
	private void validateRefundDateIsAfterLastRepayment(final LocalDate refundTransactionDate)
	{
		...
		if (possibleNextRefundDate == null || refundTransactionDate
			.isBefore(
				possibleNextRefundDate)) { throw new InvalidRefundDateException(refundTransactionDate.toString()); }
		...
		final LoanRepaymentScheduleInstallment installment)
	{
		...
		interestForCurrentPeriod = Money.of(getCurrency(), BigDecimal
			.valueOf(calculateInterestForDays(totalPeriodDays,
	...
	public void updateInstallmentsPostDate(LocalDate transactionDate)
	{
		...
		LoanRepaymentScheduleInstallment newInstallment = new LoanRepaymentScheduleInstallment(null,
			...
			installmentStartDate, transactionDate, totalPrincipal.getAmount(), null, balances[0].getAmount(),
